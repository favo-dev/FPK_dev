...
#%%
    ...
    points = ast.literal_eval(rules[10].get("value"))
    ...
    seen = set()
    unique = []
    for row in sprint_standings:
        name = row[0]             # cambia indice se il nome è in un'altra colonna
        if name not in seen:
            seen.add(name)
            unique.append(row)

    sprint_standings = unique
...
points = ast.literal_eval(rules[2].get("value"))
while len(points) < len(called):
    points.append(0)
...
for row in standings:
    key = tuple(row)           # converto in tuple per renderlo hashable
    if key not in seen:
        seen.add(key)
        unique.append(row)
