...
convocated_path = (
    "C:/Users/andre/OneDrive/Documenti/FM/Results_repository/F1/"
    + tag[:-2]  # usa il tag originale senza year per il path se serve (tu usavi tag prima dell'aggiunta anno)
    + "/called.pkl"
)
# Nota: nella versione originale tu cambiavi `tag` prima di costruire il path di called.pkl;
# per mantenere il comportamento originale ho utilizzato tag[:-2] per usare il codice che hai fornito.
# Se i tuoi file sono in directory con il tag + year, sostituisci tag[:-2] con tag.
...
def fetch_fields(record_id, fields):
    ...
    res = supabase.table("racers_f1").select(fields_str).eq("ID", record_id).execute()
    if not res.data:
...
def update_record_lists(record_id, updates):
    ...
    if changed:
        upd_res = supabase.table("racers_f1").update(payload).eq("ID", record_id).execute()
...
# SPRINT RACE
stag = "S"+tag
tag_list = normalize_to_list(stag)
